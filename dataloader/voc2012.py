...
class VOC2012_Dataset(BaseDataSet):
    ...
    def __init__(self, root, train=True, seg=False, size=(500,500)):
        ...
        #     transforms.Normalize(mean=self.MEAN, std=self.STD)
        # ])
        super(VOC2012_Dataset, self).__init__(root)
    ...
    def _load_data(self, index):
        ...
        if self.seg:
            ...
            # mask = pad_zero(mask, self.size)
            mask  = torch.squeeze(transforms.ToTensor()(pad_zero(mask,self.size)))
            return Pair(self.transform(pad_zero(image,self.size)), mask)
...
class VOC2012_DataLoader(BaseDataLoader):
    
    def __init__(self, dataset, batch_size, shuffle, num_workers, sampler=None):
        super(VOC2012_DataLoader, self).__init__(dataset, batch_size, shuffle, num_workers, sampler)
...
if __name__ == "__main__":
    ...
    # datset_classification = VOC2012_Dataset(root, seg=False)
    ...
    # # test for seg, DONE
    # dataset_seg = VOC2012_Dataset(root, seg=True)
    ...
    # dataset = VOC2012_Dataset(root, seg=True)
    # loader = VOC2012_DataLoader(dataset, batch_size=32, shuffle=True, num_workers=0)
